...
export default function AdminDashboard() {
  ...
  // Predefined gradients map to avoid Tailwind purge issues
  const gradientMap = {
    Users: "from-[#1A2B49] to-[#2F3E64]",
    Car: "from-[#4BB543] to-[#2E8B57]",
    AlertTriangle: "from-[#FF6B6B] to-[#C70039]",
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
        {stats.map((card, i) => {
          ...
          return (
            <div
              key={i}
              className={`bg-gradient-to-br ${gradientMap[card.icon]} p-6 rounded-2xl shadow-lg border border-white/10 flex items-center justify-between text-white hover:scale-[1.03] hover:shadow-2xl transform transition-all duration-300`}
            >
              <div>
